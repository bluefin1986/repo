...
public class ZoomBucksOperator implements Operator<ZoomBucksAccount> {
	...
	public void signInToTaskSite(WebDriver driver, ZoomBucksAccount zaccount)
		...
		driver.findElement(By.id("auth_central_login_username")).sendKeys(
				zaccount.getEmail());
		...
		if ("Login failed. Username or password is incorrect."
				.equals(alertContent.getText())) {
			driver.findElement(By.id("auth_central_account_name")).sendKeys(
					zaccount.getFullName());
			driver.findElement(By.id("auth_central_account_email")).sendKeys(
					zaccount.getEmail());
			...
			driver.findElement(By.id("idDiv_PWD_UsernameExample")).sendKeys(
					zaccount.getEmail());
			driver.findElement(By.id("idDiv_PWD_PasswordExample")).sendKeys(
					zaccount.getPassword());
			driver.findElement(By.id("idSIButton9")).click();

			while (true) {
				try {
					WebElement trashLink = driver.findElement(By
			...
			List<WebElement> messages = messageListContainer.findElements(By
					.xpath("div/ul/li"));
			for (WebElement msg : messages) {
				WebElement sender = msg.findElement(By.cssSelector("span.Lt"))
						.findElement(By.xpath("span/div/a/span"));
				WebElement title = msg.findElement(By.cssSelector("span.Sb"))
						.findElement(By.tagName("a"));
				if ("support@crowdflower.com".equals(sender
						.getAttribute("email"))) {
					if ("Confirmation instructions‏".equals(title.getText())) {
						while (true) {
							try {
								title.click();
								...
								WebElement confirmAnchr = msgBody
										.findElement(By.tagName("a"));
								driver.get(confirmAnchr.getAttribute("href"));
							} catch (Exception e) {
